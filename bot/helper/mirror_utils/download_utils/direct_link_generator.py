...
from bot import LOGGER, UPTOBOX_TOKEN, K2S_REFRESH_TOKEN, K2S_ACCESS_TOKEN
...
def keep2share(url: str) -> str:
    if K2S_ACCESS_TOKEN is None:
        LOGGER.error('K2S_ACCESS_TOKEN not provided!')
        dl_url = url
    else:
        try:
            if url[-1] == '/': url = url[:-1]
            file_id = url.split('/')[-1]
            cookies = {
                'accessToken': K2S_ACCESS_TOKEN,
                'refreshToken': K2S_REFRESH_TOKEN,
            }
            dl_url = rget(f'https://api.k2s.cc/v1/files/{file_id}/download', cookies=cookies).json()['downloadUrl']
        except:
